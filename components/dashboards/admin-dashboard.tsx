...
const StatusUpdateModal: React.FC<StatusUpdateModalProps> = ({ 
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="sm:max-w-md bg-gray-900 border-yellow-500 text-white">
                <DialogHeader>
                    <DialogTitle className="flex items-center text-yellow-400"><Repeat2 className="h-5 w-5 mr-2" /> Update Status for Project #{order.id}</DialogTitle>
                ...
                <div className="py-4 space-y-4">
                    ...
                    <Select value={newStatus} onValueChange={setNewStatus} disabled={isUpdating}>
                        ...
                        <SelectContent className="bg-gray-800 border-yellow-500 text-white">
                            {ORDER_STATUSES.map(status => (
                                <SelectItem key={status} value={status} className="text-white hover:bg-yellow-500 hover:text-black">
                                    {status.charAt(0).toUpperCase() + status.slice(1).replace(/_/g, ' ')}
                                </SelectItem>
                            ))}
                        </SelectContent>
                    ...
                    {/* CONDITIONAL GENERATED ORDER ID INPUT */}
                    {isGeneratedIdRequired && (
                        <div className="flex flex-col space-y-2 mt-4 p-3 bg-red-900/30 border border-red-500 rounded-md">
                            <label htmlFor="generated-id" className="text-sm font-medium text-red-300">
                                <AlertTriangle className="h-4 w-4 inline mr-1" /> Generated Order ID (Required to start project)
                            </label>
                            <Input
                                id="generated-id"
                    ...
                    {(newStatus === 'in_progress' && order.generated_order_id && !isGeneratedIdRequired) && (
                        <div className="text-sm text-yellow-300">
                            Existing Generated ID: <span className="font-semibold text-yellow-400">{order.generated_order_id}</span>
                        </div>
                    )}
                </div>
                <DialogFooter>
                    <Button variant="outline" onClick={onClose} disabled={isUpdating} className="border-yellow-500 text-yellow-500 hover:bg-yellow-500 hover:text-black">
                        Cancel
                    </Button>
                    <Button 
                        onClick={onSave} 
...
const ImageUploadModal: React.FC<ImageUploadModalProps> = ({ isOpen, onClose, order }) => {
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto bg-gray-900 border-yellow-500 text-white">
                <DialogHeader>
                ...
                {error && (
                    <div className="p-3 bg-red-900/30 border border-red-500 text-red-300 rounded text-sm">{error}</div>
                )}
                
                <div className="mt-2">
                    <h4 className="font-semibold mb-3 text-yellow-400">Project Images ({images.length})</h4>

                    {isLoading ? (
                        <div className="text-center py-8">
                            <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-yellow-500 mx-auto"></div>
                            <p className="mt-2 text-sm text-yellow-300">Loading images...</p>
                        </div>
                    ) : images.length === 0 ? (
                        <p className="text-center text-yellow-300/80 p-8 border border-yellow-500/20 rounded-lg">No images found for this project.</p>
                    ) : (
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                            {images.map((img, index) => (
                                <Card key={img.id} className="relative group overflow-hidden shadow-sm bg-gray-800 border-yellow-500/20">
                                    <div className="aspect-square w-full bg-gray-700">
                                        ...
                                        <div className="absolute inset-0 bg-black bg-opacity-40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity space-x-2">
                                            <a href={img.image_url} target="_blank" rel="noopener noreferrer">
                                                <Button variant="secondary" size="icon" title="View Image" className="bg-yellow-500 text-black hover:bg-yellow-600">
                                                    <Eye className="h-5 w-5" />
                                                </Button>
                                            </a>
                                            <Button 
                                    ...
                                    <div className="p-3 text-sm">
                                        <p className="font-medium truncate text-white">{img.description || `Image ${img.id}`}</p>
                                        <p className="text-xs text-yellow-300/80 mt-1">Uploaded: {new Date(img.created_at).toLocaleDateString()}</p>
                                    </div>
                                </Card>
                            ))}
                        </div>
                ...
                <DialogFooter className="pt-4">
                    <Button variant="outline" onClick={onClose} className="border-yellow-500 text-yellow-500 hover:bg-yellow-500 hover:text-black">
                        Close
                    </Button>
                </DialogFooter>
            </DialogContent>
...
const navItems = [
    { value: "overview", label: "Overview", icon: BarChart3 },
    { value: "users", label: "User Management", icon: Users },
    { value: "orders", label: "Project Management", icon: FolderOpen },
    { value: "tasks", label: "Tasks", icon: CheckSquare },
    { value: "attendance", label: "Staff Attendance", icon: Calendar }, 
    { value: "expenses", label: "Daily Expenses", icon: DollarSign }, 
    { value: "system", label: "System Health", icon: Activity },
...
export function AdminDashboard() {
    ...
    // Orders and tasks state (unchanged)
    const [orders, setOrders] = useState<Order[]>([])
    const [tasks, setTasks] = useState<DetailedTask[]>([])
    const [staffList, setStaffList] = useState<Staff[]>([])
    const [searchTerm, setSearchTerm] = useState("")

    // Task filter state (unchanged)
    const [taskSearchTerm, setTaskSearchTerm] = useState("")
    const [taskStaffFilterName, setTaskStaffFilterName] = useState("all")
    const [taskStatusFilter, setTaskStatusFilter] = useState("all")
    const [taskFromDate, setTaskFromDate] = useState("")
    ...
    // Data loading and handlers remain exactly the same...
    // [All your existing data loading and handler functions remain unchanged]
    const loadData = async () => {
        try {
            setIsLoading(true)
            setError('')
    ...
    // Task filters renderer with updated styling
    const renderTaskFilters = (isMobile: boolean) => (
        <>
            <div className="relative flex-1 min-w-[150px]">
                ...
                <Input 
                    placeholder="Search tasks, staff, or order ID..." 
                    className="pl-10 bg-gray-800 border-yellow-500/20 text-white placeholder-yellow-300/60" 
                    value={taskSearchTerm}
                    onChange={(e) => setTaskSearchTerm(e.target.value)}
                />
            </div>
            ...
            <Select value={taskStaffFilterName} onValueChange={setTaskStaffFilterName} disabled={isLoading}>
                <SelectTrigger className="w-full md:w-[150px] flex-shrink-0 bg-gray-800 border-yellow-500/20 text-white">
                    <SelectValue placeholder="Assigned Staff" />
                </SelectTrigger>
                <SelectContent className="bg-gray-800 border-yellow-500 text-white">
                    <SelectItem value="all" className="text-white hover:bg-yellow-500 hover:text-black">All Staff</SelectItem>
                    {staff.map(s => (<SelectItem key={s.id} value={s.staff_name} className="text-white hover:bg-yellow-500 hover:text-black">{s.staff_name}</SelectItem>))}
                </SelectContent>
            </Select>
            ...
            <Select value={taskStatusFilter} onValueChange={setTaskStatusFilter}>
                ...
                <SelectContent className="bg-gray-800 border-yellow-500 text-white">
                    <SelectItem value="all" className="text-white hover:bg-yellow-500 hover:text-black">All Statuses</SelectItem>
                    {TASK_STATUSES.map(status => (
                        <SelectItem key={status} value={status} className="text-white hover:bg-yellow-500 hover:text-black">
                            {status.charAt(0).toUpperCase() + status.slice(1).replace('_', ' ')}
                        </SelectItem>
            ...
            <Input
                ...
                value={taskFromDate}
                onChange={(e) => setTaskFromDate(e.target.value)}
                className="w-full md:w-[150px] flex-shrink-0 bg-gray-800 border-yellow-500/20 text-white"
            />
            <Input
                type="date"
                placeholder="Due To Date"
    ...
    return (
        <DashboardLayout title="Admin Dashboard" role="admin">
            <main className="flex-1 space-y-6 p-4 overflow-y-auto bg-gray-900">
                
                {isLoading && activeTab !== 'tasks' && activeTab !== 'orders' ? (
                    <div className="flex items-center justify-center py-12">
                        <div className="text-center">
                            <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-yellow-500 mx-auto mb-4"></div>
                            <p className="text-sm text-yellow-300">Loading dashboard...</p>
                        </div>
                    </div>
                ) : (
                    <>
                        ...
                        >
                            ...
                            {/* Vertical Tabs List - Updated with Black & Yellow Theme */}
                            <TabsList className="hidden md:flex flex-col p-2 h-auto space-y-1 bg-gray-800 border border-yellow-500/20 rounded-xl min-w-[200px] self-start shadow-lg shadow-yellow-500/10">
                                {navItems.map((item) => {
                                    const Icon = item.icon
                                    return (
                                        <TabsTrigger 
                                            key={item.value} 
                                            value={item.value} 
                                            className="w-full justify-start data-[state=active]:bg-yellow-500 data-[state=active]:text-black data-[state=active]:shadow-sm data-[state=active]:font-semibold rounded-lg px-3 py-2 text-sm text-yellow-400 transition-all hover:bg-yellow-500/10 hover:text-yellow-300"
                                        >
                                            <Icon className="h-4 w-4 mr-3" />
                                            {item.label}
                                        </TabsTrigger>
                            ...
                            <div className="flex-1 min-w-0 h-[calc(100vh-150px)] overflow-y-auto">
                                ...
                                {/* Overview Tab - Updated with Black & Yellow Theme */}
                                <TabsContent value="overview" className="mt-0 space-y-6">
                                    <Card>
                                        <CardHeader>
                                            <CardTitle className="flex items-center text-yellow-400"><BarChart3 className="h-5 w-5 mr-2" /> System Overview</CardTitle>
                                            <CardDescription className="text-yellow-300/80">Key metrics for operations and staffing.</CardDescription>
                                        </CardHeader>
                                        <CardContent>
                                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                                                {systemMetrics.map((metric) => {
                                                    const Icon = metric.icon
                                                    return (
                                                        <Card key={metric.name} className="p-3 bg-gray-800 border-yellow-500/20">
                                                            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 p-0">
                                                                <CardTitle className="text-sm font-medium text-yellow-300">{metric.name}</CardTitle>
                                ...
                                {/* Other tabs remain functionally the same, styling handled in their components */}
                                <TabsContent value="users" className="mt-0 space-y-6">
                                    <StaffManagementPage
                                        staff={staff}
                                        isLoading={isLoading}
                                ...
                                <TabsContent value="system" className="mt-0 space-y-6">
                                    <Card className="bg-gray-800 border-yellow-500/20">
                                        <CardHeader>
                                            <CardTitle className="text-yellow-400">System Health</CardTitle>
                                        </CardHeader>
                ...
                {/* Shared Modals - Styling updated in the modal components above */}
                <StaffForm
                    isOpen={isStaffFormOpen}
                    onClose={() => setIsStaffFormOpen(false)}
                    onSuccess={handleStaffFormSuccess}
